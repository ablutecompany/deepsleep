  ...
  if (!isOpen) {
    return (
      <div 
        ...
        style={{ position: 'fixed', bottom: '24px', right: '24px', padding: '8px 16px', borderRadius: '32px', background: 'rgba(2, 2, 3, 0.8)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', zIndex: 9999, backdropFilter: 'blur(10px)', color: '#94A3B8', fontSize: '11px', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}
      >
        <Bug size={14} color="#38BDF8" /> BETA TOOLS
      </div>
    );
  }
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', padding: '12px', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '10px', background: clockInfo.simulated ? 'rgba(245, 158, 11, 0.2)' : 'rgba(16, 185, 129, 0.2)', color: clockInfo.simulated ? '#F59E0B' : '#10B981', padding: '2px 6px', borderRadius: '4px' }}>
              {clockInfo.simulated ? `TESTE ABERTO (+${clockInfo.offset}d)` : 'TEMPO NORMAL'}
            </span>
          </div>
          ...
          <button onClick={() => appClock.reset(true)} style={btnStyle('#F43F5E', 'transparent', true)}><CalendarX2 size={14}/> Reset e Purgar Futuro</button>
        </div>
